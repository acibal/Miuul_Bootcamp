...
df.head()
df = df.select_dtypes(include=["int", "float"]) # num_cols
df = df.dropna() # notnull
df.head()
...
df[df_scores < th].shape # shape or outliers
df.describe().T # finding out why they are outliers

df.describe([0.01, 0.05, 0.75, 0.90, 0.99]).T

df.head()
df[df_scores < th].index
df[df_scores < th].drop(axis=0, labels=df[df_scores < th].index) # çok değişkenli aykırı değerler silinir.
...
# basic imputation
dataframe[col_name].fillna(dataframe[col_name].mean()) # num_col
dataframe[col_name].fillna(dataframe[col_name].median)) # num_col
dataframe[col_name].fillna(dataframe[col_name].mode()[0]) # cat_col
