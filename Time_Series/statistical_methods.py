...
arima_model = ARIMA(train, order=(1, 0, 1), trend="t").fit() #d=1 trend'i yok eder.
...
arima_model = ARIMA(train, order=best_params_aic, trend="t").fit()
y_pred = arima_model.get_forecast(48).predicted_mean
...
model = SARIMAX(train, order=(1, 0, 1), seasonal_order=(0, 0, 0, 12)) # 12 değeri m=12 aylık
# SARIMAX aslında dışsal bir parametre de girilebilen bir fonksiyon.
# bizim sadece co2 değişkenimiz var. O yüzden dışsal değişken girmedik.
# dışsal değişken girilmeyince bunu aslında SARIMA olarak kullanıyoruz.
...
# tahnminlerin hatasını da zamanla verileri kıyaslayarak yapabiliriz.
# 6 ay sonra tekrar düzeltmeleri yaptırıp yeni bir model kurabiliriz.

#### Zaman serilerinde modeli test etmek için train test ayrımı yapıyoruz. ama her zaman
#### final modelini tüm veri setiyle kuruyoruz. Çünkü son tarihten sonrasını tahmin edeceğiz.
