...
y = y['co2'].resample('MS').mean() #haftalık olan zaman dilimini aylık zaman dilimine çevirdik.

y.isnull().sum()

y = y.fillna(y.bfill()) #zaman serilerinde eksikler önceki-sonraki değer/değerler ort. ile doldurulur.
...
ses_optimizer(train, alphas, step=48)
...
des_model = ExponentialSmoothing(train, trend="add").fit(smoothing_level=0.5,
                                                         smoothing_trend=0.5)
# burda trend="add" yaptık çünkü grafikte mevsimsellik ve artıklar sıfır etrafında toplanmıştır.
# trend mevsimselliği ve artıkları etkileyememiştir. trend="add" girilir.
# ama hem çarpımsal (mul) hem toplamsal (add) olarak deneyim hatası düşük olanı seçebiliriz.
