...
# ***** Varyanslarda Ho'ı REDDETMEME, Hipotezde ise Ho'ı REDDETMEYE bakarız. *****
...
# test_stat, pvalue = shapiro(df[df["smoker"] == "Yes"]["total_bill"]) alternatif seçim
...
df = pd.read_csv("Miuul/DS_Bootcamp_13/datasets/diabetes.csv")
df.head()
...
df = pd.read_csv("Miuul/DS_Bootcamp_13/datasets/course_reviews.csv")
df.head()
...
# df[(df["sex"] == "female") & (df["survived"] == 1)]["sex"].count() alternatif seçim
type(df.loc[df["sex"] == "female", "survived"].sum())
