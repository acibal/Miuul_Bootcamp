...
df_ = pd.read_csv("DS_Bootcamp_13/case_studies/machine_learning/Case_Study_6/Paylaşılan/flo_data_20k.csv")
df = df_.copy()

date_columns = df.columns[df.columns.str.contains("date")]
df[date_columns] = df[date_columns].apply(pd.to_datetime)

# Belirli bir analiz tarihi belirleme
df[date_columns].max()
analysis_date = dt.datetime(2021, 6, 1)

# 'recency' hesaplaması, en son kaç gün önce alışveriş yapıldığını gösterir
df["recency"] = (analysis_date - df["last_order_date"]).dt.days

# 'tenure' hesaplaması, ilk ve son sipariş tarihleri arasındaki gün sayısını gösterir
...
"""`check_skew` fonksiyonunun amacı, verilen bir veri setinin belirli bir sütunundaki veri dağılımının çarpıklığını (skewness) hesaplamaktır. Çarpıklık, veri dağılımının simetrisinin bir ölçüsüdür ve bir veri setinin ne kadar asimetrik olduğunu gösterir. İşte bu konseptin bazı detayları:

1. **Çarpıklık (Skewness) Nedir?**:
   - Çarpıklık, bir veri dağılımının merkezi eğiliminin (örneğin, ortalamanın) etrafında ne kadar simetrik olduğunu ölçer. 
   - Pozitif bir çarpıklık değeri, veri dağılımının sağa doğru (pozitif değerler yönünde) çarpık olduğunu gösterir. Bu durumda, dağılımın kuyruğu sağa doğru daha uzundur.
   - Negatif bir çarpıklık değeri, veri dağılımının sola doğru (negatif değerler yönünde) çarpık olduğunu gösterir. Bu durumda, dağılımın kuyruğu sola doğru daha uzundur.
   - Çarpıklık değeri sıfıra yakınsa, bu, veri dağılımının nispeten simetrik olduğunu gösterir.

2. **`check_skew` Fonksiyonunun İşlevi**:
   - Fonksiyon, bir sütundaki tüm veri değerlerini alır ve çarpıklık değerini hesaplar. 
   - Çarpıklık testi (`stats.skewtest`), hesaplanan çarpıklık değerinin istatistiksel olarak anlamlı olup olmadığını belirler. Yani, bu test, dağılımın normal bir dağılımdan (çarpıklık = 0) önemli ölçüde farklı olup olmadığını test eder.
   - Son olarak, fonksiyon `sns.distplot` ile sütunun dağılımını görselleştirir. Bu grafik, veri dağılımının şeklini ve çarpıklığını gözle görülür bir şekilde sunar.

Bu fonksiyon, veri ön işleme ve analiz sürecinde önemlidir çünkü çarpık veriler bazı 
istatistiksel analizlerde ve makine öğrenimi modellerinde sorunlara yol açabilir. 
Örneğin, birçok makine öğrenimi modeli, veri dağılımlarının normal (Gauss dağılımı) olduğu varsayımı 
üzerine kuruludur. Çarpık veriler, bu modellerin performansını olumsuz etkileyebilir 
ve verilerin dönüştürülmesini gerektirebilir."""
