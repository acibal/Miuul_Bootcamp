...
train = pd.read_csv("DS_Bootcamp_13/case_studies/machine_learning/Case_Study_4/Paylaşılan/train.csv")
test = pd.read_csv("DS_Bootcamp_13/case_studies/machine_learning/Case_Study_4/Paylaşılan/test.csv")
len(train)
len(test)

# DataFrame'leri birleştir
df = train._append(test, ignore_index=True).reset_index()
len(df)
df.head()
...
# Ya da direkt pd.concat kullanabiliriz.
...
"""NEW_1st*GrLiv: 1stFlrSF (ilk katın alanı) ve GrLivArea (brüt yaşam alanı) değişkenlerinin çarpımı. Bu değişken, evin ilk katının yaşam alanına oranını gösterir, genellikle evin genel büyüklüğü hakkında fikir verir.

NEW_Garage*GrLiv: GarageArea (garaj alanı) ve GrLivArea (brüt yaşam alanı) değişkenlerinin çarpımı. Garaj alanının yaşam alanına oranı, evin kullanım alanı ve fonksiyonelliği hakkında bilgi sağlar.

TotalQual: Evin çeşitli özelliklerinin ve durumunun toplam puanı. Kalite ve durumla ilgili birçok değişkenin toplamıdır ve genel kaliteyi yansıtır.

NEW_TotalFlrSF: 1stFlrSF ve 2ndFlrSF (ikinci katın alanı) toplamı. Evin toplam kat alanını gösterir.

NEW_TotalBsmtFin: BsmtFinSF1 ve BsmtFinSF2 (bitmiş bodrum alanlarının büyüklükleri) toplamı. Bitmiş bodrumun toplam alanını gösterir.

NEW_PorchArea: Evin farklı veranda alanlarının (açık, kapalı, ekranlı verandalar ve üç mevsimlik veranda) toplamı. Evin dış yaşam alanının genişliğini gösterir.

NEW_TotalHouseArea: NEW_TotalFlrSF ve TotalBsmtSF (toplam bodrum alanı) toplamı. Evin toplam iç alanını gösterir.

NEW_TotalSqFeet: GrLivArea ve TotalBsmtSF toplamı. Evin toplam kullanılabilir iç alanını gösterir.

NEW_LotRatio: GrLivArea / LotArea (arsa alanı). Evin arsa alanına oranını gösterir, yoğunluğu ve alan kullanımını yansıtır.

NEW_RatioArea: NEW_TotalHouseArea / LotArea. Evin toplam alanının arsa alanına oranı.

NEW_GarageLotRatio: GarageArea / LotArea. Garaj alanının arsa alanına oranı.

NEW_MasVnrRatio: MasVnrArea (duvar kaplama alanı) / NEW_TotalHouseArea. Dış cephe kaplamasının evin toplam alanına oranı.

NEW_DifArea: LotArea - 1stFlrSF - GarageArea - NEW_PorchArea - WoodDeckSF (ahşap güverte alanı). Kullanılmayan arsa alanının hesaplanması.

NEW_OverallGrade: OverallQual (genel kalite) ve OverallCond (genel durum) çarpımı. Evin genel kalite ve durumunun birleşik değerlendirmesi.

NEW_Restoration: YearRemodAdd (yenileme tarihi) - YearBuilt (inşa tarihi). Evin yenilenme yaşı.

NEW_HouseAge: YrSold (satış yılı) - YearBuilt. Evin yaşı.

NEW_RestorationAge: YrSold - YearRemodAdd. Evin en son yenilenmesinden bu yana geçen süre.

NEW_GarageAge: GarageYrBlt (garajın inşa edildiği yıl) - YearBuilt. Garajın yaşı.

NEW_GarageRestorationAge: GarageYrBlt - YearRemodAdd mutlak değeri. Garajın yenilenme yaşı.

NEW_GarageSold: YrSold - GarageYrBlt. Garajın inşa edildiği yıldan satış yılına kadar olan süre."""
...
"""
grab_col_names(df) Fonksiyonu: Bu fonksiyon, verilen df (DataFrame) içindeki değişkenleri (sütunları) kategorik, 
sayısal ve kardinal (çok sayıda benzersiz kategoriye sahip) olmak üzere ayırıyor. Bu ayrım, veriler üzerinde uygun 
ön işleme adımlarının uygulanması için önemlidir.

Label Encoding:
label_encoder fonksiyonu, ikili (binary) kategorik değişkenler için label encoding uygular. 
Label encoding, kategorik değişkenlerdeki her bir benzersiz kategoriyi sayısal bir değere dönüştürür.
Bu kod parçasında, DataFrame'deki ikili kategorik değişkenler (binary_cols) tespit edilir ve 
her birine label_encoder fonksiyonu uygulanır. Örneğin, bir sütunda 'Evet' ve 'Hayır' değerleri varsa, 
bunlar 0 ve 1 gibi sayısal değerlere dönüştürülür.

One-Hot Encoding:
one_hot_encoder fonksiyonu, kategorik değişkenler için one-hot encoding uygular. One-hot encoding, 
her bir kategorik değişken için benzersiz kategoriler kadar yeni sütun oluşturur ve her sütunda sadece bir 
tane 1 (diğerleri 0) olacak şekilde dönüşüm yapar.
Bu işlem, modelleme sırasında kategorik değişkenlerin doğru bir şekilde kullanılmasını sağlar çünkü 
çoğu makine öğrenmesi algoritması sayısal girdilerle çalışır.

drop_first=True parametresi, her kategorik değişken için oluşturulan yeni sütunlardan 
birinin düşürülmesini sağlar. Bu, çoklu doğrusallık problemlerini önlemek için yapılır. Örneğin, bir sütunda üç kategori varsa (A, B, C), 
one-hot encoding sonucunda A, B ve C için üç sütun oluşturulur, ancak drop_first=True ile bu sütunlardan biri (örneğin A) düşürülür.
Bu ön işleme adımları, kategorik verileri makine öğrenmesi modellerinin anlayabileceği sayısal formata dönüştürmek için önemlidir. 
Label encoding genellikle ikili kategorik değişkenler için tercih edilirken, one-hot encoding çok kategorili değişkenlerde kullanılır. 
Bu teknikler, modelin kategorik verileri daha iyi anlamasını ve yorumlamasını sağlar, böylece daha doğru tahminler yapılmasına olanak tanır."""
...
RMSE: 42558.1491 (LR) 
RMSE: 47557.3947 (KNN) 
RMSE: 40721.8473 (CART) 
RMSE: 28940.2227 (RF) 
RMSE: 25984.5176 (GBM) 
RMSE: 27577.7231 (XGBoost) 
RMSE: 25717.2857 (CatBoost) 
"""
...
######## en sonda catboost ile devam ettim ########
