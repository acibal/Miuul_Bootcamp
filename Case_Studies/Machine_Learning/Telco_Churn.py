...
df = pd.read_csv("DS_Bootcamp_13/case_studies/machine_learning/Case_Study_3/paylaşılan/Telco-Customer-Churn.csv")
df.head()
df.shape
df.info()
df.dtypes
...
# df["Churn"] = [1 if col == "Yes" else 0 for col in df["Churn"]]
...
rf_best_grid.best_params_ # {'max_depth': None, 'max_features': 3, 'min_samples_split': 15, 'n_estimators': 200}
#rf_final = rf_model.set_params(rf_best_grid.best_params_, random_state=17).fit(X, y)

rf_final = RandomForestClassifier(max_depth=None, max_features=3,min_samples_split=15,n_estimators=200,random_state=17).fit(X, y)
