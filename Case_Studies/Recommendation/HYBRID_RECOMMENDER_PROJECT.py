...
movie = pd.read_csv('Data_Science_BC/W5_Recommendation/Case_Studies/HybridRecommender-221114-235254/datasets/movie.csv')
rating = pd.read_csv('Data_Science_BC/W5_Recommendation/Case_Studies/HybridRecommender-221114-235254/datasets/rating.csv')
...
def create_user_movie_df(id=False):
    import pandas as pd
    pd.set_option('display.max_columns', None)
    pd.set_option('display.width', 500)
...
random_user = int(pd.Series(user_movie_df.index).sample(1, random_state=45).values) # warningignore kullanırlırsa uyarı vermez
# random_user = 28941
...
top_users_ratings = top_users_ratings[top_users_ratings["userId"] != random_user].sort_values("corr", ascending=False)
...
# Adım 2: Öneri yapılacak kullanıcının 5 puan verdiği  puanı en güncel olan filmin id'sinin alınız.
df = movie.merge(rating, how="left", on="movieId")
top_movie_id = df[(df["userId"] == user) & (df["rating"] == 5.0)].sort_values("timestamp",ascending=False).iloc[0, 0]
...
user_movie_df = common_movies.pivot_table(index=["userId"], columns=["movieId"], values="rating")
#user_movie_df = create_user_movie(id=True)
select_movie = user_movie_df[top_movie_id]

# Adım 4: Filtrelenen dataframe’i kullanarak seçili filmle diğer filmlerin korelasyonunu bulunuz ve sıralayınız.


corr_df = user_movie_df.corrwith(select_movie).sort_values(ascending=False)
corr_df.head(10)

# Adım 5: Seçili film’in kendisi haricinde ilk 5 film’I öneri olarak veriniz.
recommend = corr_df.drop(top_movie_id).head().reset_index()
recommend.merge(movie[["movieId", "title"]], how="left", on="movieId")[["movieId", "title"]]
