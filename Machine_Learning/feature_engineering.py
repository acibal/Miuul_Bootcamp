...
def load_application_train():
    data = pd.read_csv("must-have_func/datasets/application_train.csv")
    return data
...
def load():
    data = pd.read_csv("must-have_func/datasets/titanic.csv")
    return data
...
binary_cols = [col for col in df.columns if df[col].dtype not in [int, float]
               and df[col].nunique() == 2] #boşları yakalamaz. boş varsa label fonksiyonunda 2 olacak onlar.
...
pd.get_dummies(df, columns=["Embarked"], dummy_na=True).head() #eksiklikler de bir sınıf olsun istersek
...
dff = pd.read_csv("must-have_func/datasets/course_reviews.csv")
dff.head()
dff.info()
